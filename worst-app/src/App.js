...
import './App.css';
import largeImage from './large-image.jpg';
...
class App extends Component {
  ...
  generateMultiplicationTable() {
    ...
    for (let i = 1; i <= 10; i++) {
      for (let j = 1; j <= 10; j++) {
        table.push(<p key={`${i}-${j}`}>{i} x {j} = {i * j}</p>);
      }
    }
    return table;
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <div style={{ position: 'absolute', animation: 'inefficientAnimation 5s infinite' }}>
          Texte avec animation inefficace
        </div>

        {/* Mauvaise pratique 2 : Utilisation de CSS de polices supplémentaires */}
        <p style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>Texte avec police supplémentaire</p>

        {/* Mauvaise pratique 1 : Utilisation d'animations CSS */}
        <div className="animated" style={{ animation: 'infiniteSpin 5s infinite' }}>
          Texte avec animation
        </div>

        {/* Mauvaise pratique 5 : Redimensionnement d'images en HTML */}
        <img src={largeImage} alt="Large" className='large-image' style={{ width: '400px', height: '300px' }} />
        ...
        <img src={largeImage} alt="Description" style={{ position: 'absolute', left: '-9999px' }} />

        {/* Mauvaise pratique 5. : uses-responsive-images */}
        <img src={largeImage} alt="Non-responsive" style={{ width: '800px', height: '600px' }} />
